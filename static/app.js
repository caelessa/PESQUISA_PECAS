...
if (savedCatalog && [...catalogSelect.options].some(option => option.value === savedCatalog)) {
  catalogSelect.value = savedCatalog;
}
catalogSelect.addEventListener('change', () => {
  localStorage.setItem('selectedCatalog', catalogSelect.value);
});
...
function setSearchMode(mode) {
  ...
  applicationModeButton.setAttribute('aria-selected', String(applicationMode));
  freeModeButton.setAttribute('aria-selected', String(!applicationMode));
  voiceStatus.hidden = true;
  (applicationMode ? partTypeInput : input).focus();
}

applicationModeButton.addEventListener('click', () => setSearchMode('application'));
freeModeButton.addEventListener('click', () => setSearchMode('free'));
...
if (!SpeechRecognition) {
  ...
  recognition.addEventListener('error', event => {
    ...
    recognizedText = '';
    setVoiceState(false, messages[event.error] || 'Não foi possível reconhecer a fala. Tente novamente.');
  });

  recognition.addEventListener('end', () => {
    const shouldSearch = listening && input.value.trim().length >= 2 && recognizedText.trim();
    setVoiceState(false, shouldSearch ? 'Fala reconhecida. Pesquisando…' : voiceStatus.textContent);
...
async function performSearch(query, options = {}) {
  ...
  try {
    ...
    if (!contentType.includes('application/json')) {
      throw new Error(response.ok ? 'Resposta inválida do servidor.' : 'Servidor temporariamente indisponível. Tente novamente.');
    }
    ...
    data.results.forEach(item => {
      ...
      if (item.display_text) card.querySelector('.result-card').classList.add('focused-result');
      ...
      card.querySelector('.description').textContent = visibleDescription
        .replace(item.code, '')
        .replace(/\s*\|\s*/g, '\n')
        .trim();
...
applicationForm.addEventListener('submit', event => {
  ...
  if (!partType || !vehicle) {
    return showMessage('Preencha os campos principais', 'Informe pelo menos o tipo da peça e o veículo.');
  }
  if (year && !/^(19|20)\d{2}$/.test(year)) {
    yearInput.focus();
    return showMessage('Ano inválido', 'Digite o ano com quatro números, por exemplo: 2005.');
  }

  performSearch(
    [partType, vehicle, year, engine].filter(Boolean).join(' '),
    {structured: true}
  );
});
